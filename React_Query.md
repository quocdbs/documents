<h1 align="center" style="font-size: 35px">:book: React Query Documents :book:</h1>

# React Query

## React Query là gì?

**React Query** (giờ đã đổi tên thành **TanStack Query**) là thư viện giúp fetching, caching, synchronizing và updating dữ liệu bất đồng bộ ví dụ như data từ trả về từ api.
...
React Query dùng gì để gọi API?
React Query không đảm nhận việc gọi API, việc gọi API sẽ thực hiện thông qua các thư viện bạn dùng như axios, fetch API. Còn React Query chỉ đảm nhận việc quản lý data và trigger khi cần thiết.
